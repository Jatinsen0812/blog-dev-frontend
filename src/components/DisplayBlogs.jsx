            ...
            <div key={blog._id} className="w-full my-10 flex justify-between max-xsm:flex-col ">
              <div className="w-[60%] flex flex-col gap-2 max-xsm:w-full">
                <div className="flex items-center gap-2">
                  ...
                  <p className=" hover:underline text-xl font-medium">{blog?.creator?.name}</p>
                </div>
                <h2 className="font-medium text-xl sm:text-2xl">{blog?.title}</h2>
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      {blog?.totalSaves?.includes(userId) ? (
                        ...
                        <i className="fi fi-rr-bookmark text-md mt-1"></i>
                      )}
                    </div>
                  </div>
                </div>
              </div>
              <div className="w-[40%] sm:w-[40%] rounded-lg max-xsm:w-full">
                <img
                  src={blog?.image}
                  alt=""
